...
const event: Event<Events.GuildMemberRemove> = {
    ...
    async execute(member: GuildMember | PartialGuildMember) {
        try {
            ...
            // 2. Fetch guild config for leave message
            ...
            // 3. Send leave message if configured
            if (config.leaveMessageChannelId) {
                try {
                    ...
                    if (channel && channel.isTextBased()) {
                        ...
                        if (messageData) {
                            ...
                            logger.debug(`Sent leave message for ${member.user?.tag || member.id} in ${member.guild.name}`);
                        }
                    }
                } catch (msgError) {
